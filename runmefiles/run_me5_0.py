...
def demo(screen):
    while True:
        ...
        for p in good_players:
            ...
            for count in list(cur.execute(f"SELECT count(*) from main_game_field WHERE x = {1000} and y = {1000} and owner_id=(select owner_id from owner where name='{p['module_name']}')")):
                if count[0] > 0:
                    delete_player(p)
                    player_died = True
            if player_died:
            ...
            for row in actions:
                ...
                if row[2] < 0 or row[2] > 100 or row[3] < 0 or row[3] > 50:
                    cur.execute(f"insert into main_game_field values ( {1000}, {1000}, (select owner_id from owner where name='{p['module_name']}'), TRUE, 0)")
                    break
                ...
                for c_row in collisions:
                    if c_row[0] == 'Food':
                        ...
                        for count in list(cur.execute("SELECT count(*) from main_game_field WHERE owner_id=(select owner_id from owner where name='Food')")):
                            if count[0] == 0:
                                cur.execute(f"insert into main_game_field values ( {1000}, {1000}, (select owner_id from owner where name='GAMEOVER'), TRUE, 0)")
