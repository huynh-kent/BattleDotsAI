...
class DotAgent:
    def __init__(self, cursor):
        ...
        self.cur = cursor
        self.name = 'playersTraining.050698203.gorillas_Train'
    ...
    def play_step(self, newpos_x, newpos_y, state):
        ...
        for count in list(self.cur.execute(f"select count(*) from main_game_field WHERE x = {newpos_x} and y = {newpos_y} and owner_id=(select owner_id from owner where name='Food')")):
            if count[0] > 0:
                # get food asap
                reward = int(100 * (1.0/float(num_moves+1)))
                #reward = 100
                score += 1
                num_moves = 0
...
def train(db_cursor, state):
    ...
    for row in all_rows:
        ...
        if done:
            ...
            #total_score += score
            #avg_score = total_score / num_games # score per game
