...
class DotAgent:
    def __init__(self, cursor):
        ...
        self.model = Linear_QNET(20, 256, 9)
    ...
    def get_nearestFood(self, pos_x, pos_y):
        ...
        for food in list(self.cur.execute(f"SELECT x,y,owner_id, min(SQRT(({pos_x}-X)*({pos_x}-X) + ({pos_y}-Y)*({pos_y}-Y))) from main_game_field WHERE owner_id=(select owner_id from owner where name='Food')")):
            nearestfood = food
    ...
    def get_nearest_ally(self, pos_x, pos_y):
        for ally in list(self.cur.execute(f"select x,y, min(SQRT(({pos_x}-X)*({pos_x}-X) + ({pos_y}-Y)*({pos_y}-Y))) from main_game_field where owner_id=(select owner_id from owner where name='players4_0.001.DetectEnemy1')")):
            nearest_ally = ally
        return nearest_ally[0], nearest_ally[1]

    def get_nearest_enemy(self, pos_x, pos_y):
        for enemy in list(self.cur.execute(f"select x,y, min(SQRT(({pos_x}-X)*({pos_x}-X) + ({pos_y}-Y)*({pos_y}-Y))) from main_game_field where owner_id<>(select owner_id from owner where name='Food') and owner_id<>(select owner_id from owner where name='players4_0.001.DetectEnemy1')")):
            nearest_enemy = enemy
        return nearest_enemy[0], nearest_enemy[1]

    def get_nearest_thing(self, pos_x, pos_y):
        food = ally = enemy = False
        for thing in list(self.cur.execute(f"select owner_id, min(SQRT(({pos_x}-X)*({pos_x}-X) + ({pos_y}-Y)*({pos_y}-Y))) from main_game_field")):
            nearest_thing = thing
        if nearest_thing[0] == 0:
            food = True
        elif nearest_thing[0] == 1:
            ally = True
        elif nearest_thing[0] == 2:
            enemy = True
        return food, ally, enemy
    ...
    def get_gamestate(self, pos_x, pos_y): 
        ...
        nearest_enemy_x, nearest_enemy_y = self.get_nearest_enemy(pos_x, pos_y)
        food, ally, enemy = self.get_nearest_thing(pos_x, pos_y)
        ...
        state = [
            ...
            nearest_enemy_x < pos_x,  # enemy left
            nearest_enemy_x > pos_x,  # enemy right
            nearest_enemy_y < pos_y,  # enemy up
            nearest_enemy_y > pos_y,  # enemy down   
    ...
    def get_action(self, state):
        ...
        self.epsilon = 0 - self.num_games # lower for loading better models
    ...
    def get_score(self):
        for count in list(self.cur.execute(f"select count(*) from main_game_field where is_flag=FALSE and owner_id=(select owner_id from owner where name='players4_0.001.DetectEnemy1')")):
            score = count[0]
        return score
    ...
    def play_step(self, newpos_x, newpos_y, state):
        ...
        # if game over from other player
        #for count in list(self.cur.execute(f"SELECT count(*) from main_game_field WHERE x = {1000} and y = {1000}")):
        #    if count[0] > 0:
        #        game_over = True

        # if our dot goes out of bounds
        for count in list(self.cur.execute(f"SELECT count(*) from main_game_field WHERE ({newpos_x} < 0 or {newpos_x} > {GAME_WIDTH} or {newpos_y} < 0 or {newpos_y} > {GAME_HEIGHT}) and owner_id=(select owner_id from owner where name='players4_0.001.DetectEnemy1')")):
            if count[0] > 0:
                game_over = True
                reward = -10
                

        # check if dot is close to enemy dot
        for count in list(self.cur.execute(f"select * from main_game_field where SQRT(({newpos_x}-X)*({newpos_x}-X) + ({newpos_y}-Y)*({newpos_y}-Y))<=SQRT(2) and owner_id<>(select owner_id from owner where name='Food') and owner_id<>(select owner_id from owner where name='players4_0.001.DetectEnemy1')")):
        ...
        for count in list(self.cur.execute(f"select * from main_game_field where SQRT(({newpos_x}-X)*({newpos_x}-X) + ({newpos_y}-Y)*({newpos_y}-Y))<=SQRT(2) and owner_id=(select owner_id from owner where name='players4_0.001.DetectEnemy1')")):
            if count is not None and count[3] == 0:
                reward = -10
                break
...
def train(db_cursor, state):
    ...
    print(f"Player1 - Score:{score} - Record:{high_score}")
