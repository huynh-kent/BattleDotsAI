...
class DotAgent:
    ...
    def get_nearest_ally(self, pos_x, pos_y):
        for ally in list(self.cur.execute(f"select x,y, min(SQRT(({pos_x}-X)*({pos_x}-X) + ({pos_y}-Y)*({pos_y}-Y))) from main_game_field where owner_id=(select owner_id from owner where name='players4_0.002.DetectEnemy2')")):
            nearest_ally = ally
        return nearest_ally[0], nearest_ally[1]

    def get_nearest_enemy(self, pos_x, pos_y):
        for enemy in list(self.cur.execute(f"select x,y, min(SQRT(({pos_x}-X)*({pos_x}-X) + ({pos_y}-Y)*({pos_y}-Y))) from main_game_field where owner_id<>(select owner_id from owner where name='Food') and owner_id<>(select owner_id from owner where name='players4_0.002.DetectEnemy2')")):
            nearest_enemy = enemy
    ...
    def get_nearest_thing(self, pos_x, pos_y):
        ...
        if nearest_thing[0] == 0:
            food = True
        elif nearest_thing[0] == 2: # our owner_id
            ally = True
        else:
            enemy = True
    ...
    def get_score(self):
        for count in list(self.cur.execute(f"select count(*) from main_game_field where is_flag=FALSE and owner_id=(select owner_id from owner where name='players4_0.002.DetectEnemy2')")):
            score = count[0]
        return score
    ...
    def play_step(self, newpos_x, newpos_y, state):
        ...
        for count in list(self.cur.execute(f"SELECT count(*) from main_game_field WHERE x = {1000} and y = {1000} and owner_id=(select owner_id from owner where name='players4_0.002.DetectEnemy2')")):
            if count[0] > 0:
                game_over = True
                reward = -10
        ...
        for count in list(self.cur.execute(f"select * from main_game_field where SQRT(({newpos_x}-X)*({newpos_x}-X) + ({newpos_y}-Y)*({newpos_y}-Y))<=SQRT(2) and owner_id<>(select owner_id from owner where name='Food') and owner_id<>(select owner_id from owner where name='players4_0.002.DetectEnemy2')")):
        ...
        for count in list(self.cur.execute(f"select * from main_game_field where SQRT(({newpos_x}-X)*({newpos_x}-X) + ({newpos_y}-Y)*({newpos_y}-Y))<=SQRT(2) and owner_id=(select owner_id from owner where name='players4_0.002.DetectEnemy2')")):
            if count is not None and count[3] == 0:
                reward = -10
                turns = self.elapsed_turns
                break
...
def train(db_cursor, state):
    ...
    print(f"Player2 - Score:{score} - Record:{high_score}")
