...
class DotAgent:
    ...
    def get_score(self):
        for count in list(self.cur.execute(f"select count(*) from main_game_field where is_flag=FALSE and owner_id=(select owner_id from owner where name='players4_0.005.MultiDots2')")):
            score = count[0]
        return score
    ...
    def play_step(self, newpos_x, newpos_y, state):
        ...
        for count in list(self.cur.execute(f"SELECT count(*) from main_game_field WHERE ({newpos_x} < 0 or {newpos_x} > {GAME_WIDTH} or {newpos_y} < 0 or {newpos_y} > {GAME_HEIGHT}) and is_flag=FALSE and owner_id=(select owner_id from owner where name='players4_0.005.MultiDots2')")):
            if count[0] > 0:
                game_over = True
                reward = -10
