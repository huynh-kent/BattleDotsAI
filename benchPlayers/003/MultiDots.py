...
class DotAgent:
    ...
    def get_score(self):
        for count in list(self.cur.execute(f"select count(*) from main_game_field where is_flag=FALSE and owner_id=(select owner_id from owner where name='players4_0.003.MultiDots')")):
            score = count[0]
        return score
    ...
    # random moves: tradeoff exploration / exploitation
        self.epsilon = 5 - self.num_games # lower for loading better models
    ...
    def play_step(self, newpos_x, newpos_y, state):
        ...
        for count in list(self.cur.execute(f"SELECT count(*) from main_game_field WHERE ({newpos_x} < 0 or {newpos_x} > {GAME_WIDTH} or {newpos_y} < 0 or {newpos_y} > {GAME_HEIGHT}) and is_flag=FALSE and owner_id=(select owner_id from owner where name='players4_0.003.MultiDots')")):
            if count[0] > 0:
                game_over = True
                reward = -10
...
def train(db_cursor, state):
    global trained_agents, high_score, plot_scores, plot_avg_scores, total_score, score, num_games
    # get all my dots
