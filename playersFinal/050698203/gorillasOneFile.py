...
class DotAgent:
    def __init__(self, cursor):
        ...
        self.model = Linear_QNET(15, 256, 9)
        self.trainer = Q_Trainer(self.model, lr=LR, gamma=self.gamma)
        self.cur = cursor
        self.name = 'playersFinal.050698203.gorillasOneFile'
...
def train(db_cursor, state):
    ...
    for row in all_rows:
        ...
        if dotAgent.check_if_food():
            ...
            on_team = False
        #    for count in list(dotAgent.cur.execute(f"SELECT count(*) from main_game_field WHERE ((x-{newpos_x})*(x-{newpos_x})) + ((y-{newpos_y})*(y-{newpos_y})) < 2 and is_flag=FALSE and owner_id=(select owner_id from owner where name='{dotAgent.name}')")):
        #        if count[0] > 0:
        #            on_team = True
